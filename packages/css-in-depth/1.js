...
/* ------------------------选择器------------------------ */
1. 组合器
~通用兄弟选择器，不能选中目标元素前的元素：li.target ~ li
li.target,复合选择器compound,  

2.伪类
// 伪类选择器(:)：选中处于 特定状态(用户交互状态、或者相对于父级|兄弟元素的位置) 的元素。
伪选择器的优先级等于class。
:nth-child(an+b)——匹配的元素在兄弟元素中间有特定的位置。公式an+b里面的a和b是整数，该公式指定要选中哪个元素。要了解一个公式的工作原理，请从0开始代入n的所有整数值。
    eg:(-n+4)：选中前4个，（n+4）从第4个开始的每个元素。
:only-child, 是父元素的唯一子元素的元素。
:empty——匹配的元素必须没有子元素。注意，如果元素包含空格就无法由该选择器匹配，因为空格在DOM中属于文本节点。W3C正在考虑:blank伪类选择器，它跟:empty的行为类似，但是能选中仅包含空格的元素，目前还没有浏览器支持:blank
:root——匹配文档根元素。对HTML来说，这是<html>元素，但是CSS还可以应用到XML或者类似于XML的文档上，比如SVG。在这些情况下，该选择器的选择范围更广。

表单域相关的伪选择器：Level4的ie10不支持，查看can i use.
...
:optional——匹配没有设置required属性的元素（Level4）。 以上并未列出全部伪类选择器。请参阅MDN文档Pseudo-classes，

3.伪元素
// 伪元素选择器(::,为了兼容浏览器支持:)：不匹配特定状态的元素，而是匹配在文档中 没有直接对应HTML 元素的 特定部分。伪元素选择器可能只匹配元素的一部分，甚至向HTML标记中未定义的地方插入内容
伪选择器的优先级等于tag。
::before|::after,可插入文字、图片或其他形状。
...
5.不区分大小写的属性选择器
选择器规范Level4提出了一种不区分大小写的修饰符，可以作用于任何属性选择器。它的用法是将i添加到结束方括号前面，例如：input[value="search"i]。
很多浏览器还不支持该特性，直接忽略。如果使用请提供回退方案。
